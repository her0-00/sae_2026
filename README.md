...
> **Projet SAÉ 2026** | Python · PostgreSQL/PostGIS · Flask · Dagster · Azure OpenAI  
> Données ouvertes : DVF (data.gouv.fr) · ADEME · INSEE · IGN · OSM · DGAC
...
8. [Assistant IA — ImmoBI Copilot](#-assistant-ia--immobi-copilot)
9. [Installation et démarrage](#-installation-et-démarrage)
10. [Déploiement](#-déploiement)
11. [Métriques de la base de données](#-métriques-de-la-base-de-données)
...
Un **assistant IA conversationnel** (ImmoBI Copilot) est intégré à l'interface : il interroge la base en temps réel via génération SQL automatique et répond en streaming SSE.
...
│  /api/opportunites · /api/communes · /api/chat                      │
└─────────────────┬───────────────────────────────────────────────────┘
                  │  JSON / GeoJSON
                  ▼
┌─────────────────────────────────────────────────────────────────────┐
│              FRONTEND (HTML · CSS · JavaScript · Leaflet · Chart.js) │
│  Estimateur · Fiche Commune · Carte · Opportunités · Architecture       │
└─────────────────────────────────────────────────────────────────────┘
```
...
| `communes` | `GET /api/communes/{code}/resume` | Fiche synthétique d'une commune |
| `chat` | `POST /api/chat` | Assistant IA — réponse en streaming SSE |
...
|------|-------------|--------------------------|
| **Analyseur de prix** | Chart.js | Estimation + verdict deal + percentile bar + analyse multi-dimensionnelle |
| **Carte des prix** | Leaflet.js | Choroplèthe · 5 métriques · bascule Plan/Satellite · transactions réelles GPS |
| **Fiche commune** | Chart.js | 5 onglets : marché, DPE, transport, bruit, contexte socio-économique |
| **Opportunités** | Vanilla JS | Scoring 100 pts · 7 critères · classement par département |
| **Architecture** | — | Documentation visuelle de la stack technique (soutenance) |
...
Chatbot conversationnel intégré à toutes les pages (bouton 💬 en bas à droite). Il interroge la base de données en temps réel et répond en langage naturel.
...
Question utilisateur
       │
       ▼
Détection de commune ──► 1 requête SQL combinée (prix + bruit + DPE)
       │
       ▼
LLM (Azure OpenAI) ──► SQL généré ──► Exécution PostgreSQL
       ▲                                    │ erreur
       └────────────────────────────────────┘ retry auto (max 3)
                             │ succès
                             ▼
LLM ──► Réponse naturelle basée sur données réelles
                             │
                             ▼
Streaming SSE ──► texte affiché mot par mot côté client
```
...
| Avant | Après |
|-------|-------|
| Client AzureOpenAI recréé à chaque requête | **Singleton** — instancié une seule fois au démarrage |
| N requêtes DB séquentielles (1 par mot ≥ 4 chars) + 3 par commune | **1 seule requête SQL combinée** (prix + bruit + DPE en sous-requêtes) |
| Attend la réponse complète avant affichage | **Streaming SSE** — premier token affiché en < 1s |
...
|-----------|-----------|-------|
| API Flask | **Render.com** | Auto-deploy sur push `master` · serveur Gunicorn |
| Base de données | **Render.com** | PostgreSQL 15 managé + extension PostGIS |
| Pipeline Dagster | **Local** | Lancé manuellement pour l'ingestion des données |
| Code source | **GitHub** | Branche `master` → déploiement automatique Render |

> **Sécurité** : Ne jamais commiter le fichier `.env` (déjà présent dans `.gitignore`). Utiliser les variables d'environnement de la dashboard Render pour la production.
